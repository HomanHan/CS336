# Thanks to https://github.com/Louisym/Stanford-CS336-spring25/blob/main/assignment1-basics/cs336_basics/train.py
# just for reference
"""
...
from cs336_basics.AdamW import learning_rate_schedule, gradient_clipping
...
def parse_args():
    ...
    parser.add_argument(
        "--context_len", type=int, default=256, help="Maximum sequence length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cosine_it", type=int, default=10000, help="Cosine annealing iterations"
    )
    ...
    parser.add_argument(
        "--train_steps", type=int, default=6000, help="Total training steps"
    )
    parser.add_argument(
        "--val_interval", type=int, default=100, help="Validation interval"
    )
    parser.add_argument(
        "--val_batches", type=int, default=10, help="Number of validation batches"
    )
    parser.add_argument(
        "--save_intervals", type=int, default=1000, help="Checkpoint save interval"
    )
    ...
    parser.add_argument(
        "--resume_ckp", type=str, default=None, help="Path to checkpoint to resume from"
    )
...
def main():
    ...
    for iter_num in pbar:
        ...
        lr = learning_rate_schedule(
            iter_num, args.max_lr, args.min_lr, args.warm_up_it, args.cosine_it
        )
        ...
        input_ids, target_ids = data_loading(
            train_data, args.batch_size, args.context_len, device=device
        )
        ...
        if iter_num % args.val_interval == 0 and iter_num > 0:
            ...
            with torch.no_grad():
                for _ in range(args.val_batches):
                    val_input_ids, val_target_ids = data_loading(
                        val_data, args.batch_size, args.context_len, device=device
                    )
            ...
            tqdm.write(
                f"Validation | Loss: {avg_val_loss:.4f} | PPL: {val_perplexity:.2f}"
            )
        ...
        if iter_num % args.save_intervals == 0 and iter_num > 0:
            checkpoint_path = os.path.join(
                args.save_ckp_path, f"checkpoint_{iter_num}.pt"
            )
    ...
    final_checkpoint_path = os.path.join(
        args.save_ckp_path, f"checkpoint_final_{args.train_steps}.pt"
    )

# Thanks to https://github.com/Louisym/Stanford-CS336-spring25/blob/main/assignment1-basics/cs336_basics/generate.py
#!/usr/bin/env python3
"""
...
def top_p_sampling(probabilities: torch.Tensor, p: float = 0.9) -> torch.Tensor:
    ...
    # Compute cumulative probabilities 从高到低做累加和，抛弃 p 之外的部分
    ...
    output_probs.scatter_(-1, sorted_indices, filtered_probs)  # 还原回原始顺序
...
def generate_text(
    ...
    input_ids = torch.tensor(prompt_tokens, dtype=torch.long, device=device).unsqueeze(
        0
    )  # unsqueeze to add batch dimension (L) to (1, L)
    ...
    with torch.no_grad():
        for _ in range(max_tokens):
            ...
            # Get logits for the last token，只取最后一个 token 的输出 logit
            next_token_logits = logits[
                0, -1, :
            ]  # batch=0, L=-1 (last), vocab=all | Shape: (vocab_size,)
            ...
            # Sample next token，抽样，返回索引 token ID。item() 将单元素张量转换为 Python 标量
...
def load_model_and_tokenizer(
    checkpoint_path: str, vocab_path: str, merges_path: str, device: str = "cpu"
):
    """
    ...
    try:
        ...
        print(
            "Warning: Could not import tokenizer. You may need to implement or adjust import path."
        )
        tokenizer = None
    ...
    model = TransformerLM(
        ...
        num_layer=config.get("num_layers", 4),
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate text with a trained Transformer language model"
    )
    ...
    parser.add_argument(
        "--vocab", type=str, required=True, help="Path to tokenizer vocabulary file"
    )
    parser.add_argument(
        "--merges", type=str, required=True, help="Path to tokenizer merges file"
    )
    ...
    parser.add_argument(
        "--top_p", type=float, default=0.9, help="Top-p threshold for nucleus sampling"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eos_token", type=str, default="<|endoftext|>", help="End-of-sequence token"
    )
    ...
    if tokenizer is None:
        print(
            "Error: Tokenizer could not be loaded. Please check your tokenizer implementation."
        )
        return
    ...
    print(
        f"Parameters: max_tokens={args.max_tokens}, temperature={args.temperature}, top_p={args.top_p}"
    )
    print("-" * 80)

...
import pandas as pd
from cs336_systems.flashattention import FlashAttentionFunc_triton
import itertools
...
def main():
    ...
    print(
        f"Testing {len(seq_lengths)} sequence lengths, {len(d_models)} embedding dims, {len(dtypes)} dtypes"
    )
    print(
        f"Total configurations: {len(seq_lengths) * len(d_models) * len(dtypes) * 2}"
    )  # *2 for flash and pytorch
    ...
    for seq_len, d_model, dtype in itertools.product(seq_lengths, d_models, dtypes):
        ...
        try:
            ...
            print(
                f"    Forward: {pytorch_result['forward_ms']:.3f}ms, Backward: {pytorch_result['backward_ms']:.3f}ms"
            )
        ...
        try:
            ...
            flash_result = benchmark_attention(
                "FlashAttention2", FlashAttentionFunc_triton.apply, seq_len, d_model, dtype
            )
            results.append(flash_result)
            print(
                f"    Forward: {flash_result['forward_ms']:.3f}ms, Backward: {flash_result['backward_ms']:.3f}ms"
            )
        except RuntimeError as e:
